...
module tb_ntt4096_four_butterfly_pipeline_transform_core;
    ...
    ntt4096_four_butterfly_pipeline_transform_core dut (
        .clk             (clk),
        .reset_n         (reset_n),
    ...
    task automatic write_modulus (
        ...
        input logic [30:0] reciprocal_value
    );
        begin
            modulus_data =
                value;
    ...
    begin
        ...
        $display(
            "START: q0 pipelined four-butterfly forward DIF"
        );

        write_modulus(Q0, MU0);
        load_coefficients(0);
        load_twiddles(0, 0);
        run_transform(0, 0);
        ...
        $display(
            "START: q0 pipelined four-butterfly inverse DIT"
        );
        ...
        $display(
            "START: q1 pipelined four-butterfly forward DIF"
        );

        write_modulus(Q1, MU1);
        load_coefficients(1);
        load_twiddles(1, 0);
        run_transform(0, 1);
        ...
        $display(
            "START: q1 pipelined four-butterfly inverse DIT"
        );
        ...
        begin
            $fatal(
                1,
                "transform_count=%0d expected=4",
                transform_count
            );
        end

        if (checked_words != 4 * N)
        begin
            $fatal(
                1,
                "checked_words=%0d expected=%0d",
                checked_words,
                4 * N
            );
        end
        ...
        $display(
            "PASS: 4 pipelined transforms, %0d checked coefficients, %0d cycles each",
            checked_words,
            EXPECTED_CYCLES
        );
