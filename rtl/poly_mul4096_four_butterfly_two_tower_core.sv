...
/*
 * Two lockstep RNS towers using the proven 22,968-cycle four-butterfly
 * pipelined polynomial core.
 *
 * Both towers share control and logical addresses. Data, modulus, reciprocal,
 * runtime profiles, and coefficient memories remain independent.
 *
 * Expected physical projection on XC7Z020:
 *
 *     DSP48E1:  128 / 220
 *     RAMB18E1:  32
 *     RAMB36E1:  64
 */
module poly_mul4096_four_butterfly_two_tower_core (
    input  logic        clk,
    input  logic        reset_n,
    input  logic        start,
    ...
    poly_mul4096_four_butterfly_pipeline_runtime_profile_core lane0 (
        .clk                       (clk),
        .reset_n                   (reset_n),
        .start                     (start),
    ...
    poly_mul4096_four_butterfly_pipeline_runtime_profile_core lane1 (
        .clk                       (clk),
        .reset_n                   (reset_n),
        .start                     (start),
...
`ifndef SYNTHESIS
    ...
    begin
        if (reset_n)
        begin
            if (lane0_busy != lane1_busy)
            begin
                $display("ERROR: two RNS towers left busy lockstep");
                $fatal(1);
            end

            if (lane0_done != lane1_done)
            begin
                $display("ERROR: two RNS towers left done lockstep");
                $fatal(1);
            end
            ...
            begin
                $display(
                    "ERROR: two RNS towers completed with different counters"
                );
                $fatal(1);
            end
        end
