...
/*
 * Complete N=4096 radix-2 DIF cyclic NTT controller.
 *
 * Input coefficients are loaded in natural order. Output coefficients
 * are stored in bit-reversed order:
 *
 *     memory[bit_reverse(k)] = transform[k]
 *
 * One butterfly is processed at a time through butterfly_dif_core.
 * The schedule entry remains fixed until both results are written.
 *
 * The same controller supports forward or inverse cyclic transforms by
 * selecting the corresponding compact twiddle initialization file.
 */
module ntt4096_dif_cyclic_core #(
    ...
    ntt4096_dif_schedule_core schedule (
        .clk          (clk),
        .reset_n      (reset_n),
...
`ifndef SYNTHESIS
    ...
    begin
        ...
        begin
            $display(
                "ERROR: N=4096 DIF scheduler became invalid state=%0d operation=%0d",
                state,
                schedule_operation
            );
        ...
        begin
            $display(
                "ERROR: N=4096 DIF butterfly start attempted while busy"
            );
