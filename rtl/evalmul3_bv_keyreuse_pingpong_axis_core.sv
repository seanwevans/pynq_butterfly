 ...
 *
 * Two coefficient banks decouple input/computation from AXI output. Once one
 * coefficient is complete, its bank becomes available to the output stream
 * while the alternate bank accepts and computes the next coefficient.
 *
 * Input and output therefore overlap whenever output backpressure does not
 * occupy both banks. The input-bound schedule remains 16B+24 words per
 * coefficient for 12 BV digits.
 *
 ...
 */
module evalmul3_bv_keyreuse_pingpong_axis_core #(
     ...
     * Two-bank ownership.
     *
     * A completed compute bank becomes ready for output. A bank is released
     * only after its final ciphertext's c1 word is accepted.
     */
    always_ff @(posedge clk)
    begin
        if (!reset_n)
    ...
    /*
     * BV metadata FIFO and pipelined per-ciphertext accumulators.
     *
     * Stage 0 consumes the fixed-latency multiplier result and metadata,
     * captures the selected accumulator's old value, and advances the FIFO.
     *
     * Stage 1 performs the modular accumulation and writes the selected bank.
     *
     * MIN_BATCH=8 guarantees that the same accumulator address is not revisited
     * before the prior staged update has committed.
     */
    always_ff @(posedge clk)
    begin
        if (!reset_n)
        ...
        begin
            /*
             * Stage 1 commit. Count final-digit results only when their
             * accumulator writes occur.
             */
            if (start_coefficient_event)
            begin
                bv_final_result_count <=
            ...
            /*
             * Stage 0 capture. The old accumulator value is registered before
             * entering the modular carry chain.
             */
            bv_accum_valid <=
                bv_meta_pop;
...
`ifndef SYNTHESIS
    ...
    begin
        ...
        begin
            if (
                start_coefficient_event
                && bv_accum_valid
            )
            begin
                $display(
                    "ERROR: started a coefficient while a BV accumulator update remained pending"
                );
            ...
            begin
                $display(
                    "ERROR: completed a coefficient into a full ping-pong bank"
                );
            ...
            begin
                $display(
                    "ERROR: released a ping-pong bank that was not ready"
                );
            ...
            begin
                $display(
                    "ERROR: simultaneous compute/output ownership collision"
                );
