...
/*
 * Eight-bank coefficient store with a dedicated idle-only eight-wide handoff
 * write source. The normal arithmetic write buses remain connected directly
 * to the per-bank write-selection logic; the handoff source is selected inside
 * this hierarchy so Vivado can place each small mux beside its destination
 * BRAM instead of routing a centralized wide mux across the device.
 */
module ntt4096_eight_bank_coeff_store_runtime_handoff (
    input  logic              clk,
    ...
    input  logic [7:0][31:0]  handoff_write_data
);
    ...
    logic [7:0][11:0] handoff_write_addr;
    ...
    generate
        ...
        begin : banks
            ntt4096_coeff_bank_512x32 bank (
                ...
                .read_en (
                    bank_read_en[bank_index]
                ),

                .read_addr (
                    bank_read_addr[bank_index]
                ),
                ...
                .write_en (
                    bank_write_en[bank_index]
                ),

                .write_addr (
                    bank_write_addr[bank_index]
                ),

                .write_data (
                    bank_write_data[bank_index]
                )
            );
        end
    endgenerate
