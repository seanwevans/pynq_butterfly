 ...
 *   BATCH frame:
 *
 *     word 0      0x4d554c42  ("MULB")
 *     word 1      batch_count, which must be positive
 *
 *     Then, for every product in the batch:
 *
 *       4096 words A[0..4095]
 *       4096 words B[0..4095]
 *
 *     Total: 2 + batch_count * 8192 words.
 *
 * Output for PRODUCT:
 *
 ...
 * Output for BATCH:
 *
 *     batch_count consecutive 4096-word result polynomials.
 *
 * TLAST is required only on the final word of the complete input
 * frame and the final coefficient of the complete output frame.
 * Intermediate products in a batch do not assert TLAST.
 */
module poly_mul4096_dual_butterfly_runtime_profile_batch_axis_core (
    input  logic        clk,
    input  logic        reset_n,
    ...
    /*
     * AXI operand writes are registered before they enter the arithmetic
     * core. This removes the wrapper FSM from the coefficient-BRAM
     * address/write-enable path while retaining one accepted word per
     * clock during operand loading.
     */
    ...
    assign final_batch_product =
        !batch_mode
        || batch_index + 1'b1 == batch_count;
    ...
    /*
     * The registered write pipeline accepts a new AXI word every clock.
     * The final B word is committed during STATE_PRODUCT_DRAIN; the
     * following STATE_PRODUCT_START cycle then launches the unchanged
     * arithmetic core.
     */
    ...
    begin
        ...
        begin
            case (state)
                ...
                begin
                    /*
                     * The final registered B write reaches the core on
                     * this edge. No AXI word is accepted in this state.
                     */
                    state <=
                        STATE_PRODUCT_START;
                end
...
`ifndef SYNTHESIS
    ...
    begin
        ...
        begin
            $display(
                "ERROR: AXI runtime-profile batch adapter started without a committed profile"
            );
        ...
        begin
            $display(
                "ERROR: AXI runtime-profile batch adapter wrote A while the core was busy"
            );
        ...
        begin
            $display(
                "ERROR: AXI runtime-profile batch adapter wrote B while the core was busy"
            );
