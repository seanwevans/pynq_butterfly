...
WORK_DIR="${BV_KEYREUSE_PINGPONG_TIMING_SWEEP_WORK:-/mnt/f/v/bv_keyreuse_pingpong_timing_sweep}"
...
required=(
  ...
  "$ROOT/scripts/vivado/build_bv_keyreuse_pingpong_timing_sweep.tcl"
)
...
tcl_windows="$repo_windows\\scripts\\vivado\\build_bv_keyreuse_pingpong_timing_sweep.tcl"

cmd_file="$WORK_DIR/run_bv_keyreuse_pingpong_timing_sweep.cmd"
...
if [[ ! -f "$summary" ]] \
  ...
  echo "error: Vivado did not record the ping-pong timing-sweep PASS marker" >&2
  status=1
fi
