...
set ip_vlnv \
    user.org:user:poly_mul4096_runtime_profile_axis:1.0

set project_root \
    F:/repos/pynq_poly_mul4096_runtime_profile_dma

set project_name \
    pynq_poly_mul4096_runtime_profile_dma
...
set deploy_dir \
    [file join $root_dir deploy poly_mul4096_runtime_profile_dma]

set report_dir \
    [file join $root_dir reports poly_mul4096_runtime_profile_dma]
...
set accelerator \
    [create_bd_cell \
        -type ip \
        -vlnv $ip_vlnv \
        poly_mul4096_runtime_profile_axis_0 \
    ]
...
connect_bd_intf_net \
    ...
    [get_bd_intf_pins poly_mul4096_runtime_profile_axis_0/s_axis]

connect_bd_intf_net \
    [get_bd_intf_pins poly_mul4096_runtime_profile_axis_0/m_axis] \
...
connect_bd_net \
    ...
    [get_bd_pins poly_mul4096_runtime_profile_axis_0/clk]
...
connect_bd_net \
    ...
    [get_bd_pins poly_mul4096_runtime_profile_axis_0/reset_n]
...
if {
    ...
    error "Runtime-profile N=4096 DMA-overlay implementation failed: $implementation_status"
}

open_run impl_1

set timing_report \
    [file join $report_dir poly_mul4096_runtime_profile_dma_timing.rpt]

set utilization_report \
    [file join $report_dir poly_mul4096_runtime_profile_dma_utilization.rpt]
...
set hwh_destination \
    [file join \
        $deploy_dir \
        poly_mul4096_runtime_profile_dma.hwh \
    ]
...
set xsa_destination \
    [file join \
        $deploy_dir \
        poly_mul4096_runtime_profile_dma.xsa \
    ]
...
puts $manifest_handle "overlay=poly_mul4096_runtime_profile_dma"
...
puts $manifest_handle "profile_frame_words=16384"
puts $manifest_handle "profile_frame_bytes=65536"
puts $manifest_handle "profile_payload_words=16382"
puts $manifest_handle "product_command=0x4d554c31"
puts $manifest_handle "product_input_frame_words=8193"
puts $manifest_handle "product_input_frame_bytes=32772"
puts $manifest_handle "product_output_frame_words=4096"
puts $manifest_handle "product_output_frame_bytes=16384"
puts $manifest_handle "coefficient_banks=2"
puts $manifest_handle "profile_banks=4"
puts $manifest_handle "core_cycles=1339394"
puts $manifest_handle "core_latency_us=13393.94"
puts $manifest_handle "modular_multiplications=90112"
puts $manifest_handle "expected_core_ramb36=24"
...
puts ""
puts "============================================================"
puts "PYNQ-Z2 N=4096 RUNTIME-PROFILE DMA OVERLAY COMPLETE"
puts "============================================================"
