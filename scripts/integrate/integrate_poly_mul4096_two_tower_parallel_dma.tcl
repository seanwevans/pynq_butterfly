...
set ip_vlnv \
    user.org:user:poly_mul4096_two_tower_parallel_axis:1.0

set project_root \
    F:/repos/pynq_poly_mul4096_two_tower_parallel_dma

set project_name \
    pynq_poly_mul4096_two_tower_parallel_dma
...
set deploy_dir \
    [file join $root_dir deploy poly_mul4096_two_tower_parallel_dma]

set report_dir \
    [file join $root_dir reports poly_mul4096_two_tower_parallel_dma]
...
set accelerator \
    [create_bd_cell \
        -type ip \
        -vlnv $ip_vlnv \
        poly_mul4096_two_tower_parallel_axis_0 \
    ]
...
connect_bd_intf_net \
    ...
    [get_bd_intf_pins poly_mul4096_two_tower_parallel_axis_0/s_axis]

connect_bd_intf_net \
    [get_bd_intf_pins poly_mul4096_two_tower_parallel_axis_0/m_axis] \
...
connect_bd_net \
    ...
    [get_bd_pins poly_mul4096_two_tower_parallel_axis_0/clk]
...
connect_bd_net \
    ...
    [get_bd_pins poly_mul4096_two_tower_parallel_axis_0/reset_n]
...
if {
    ...
    error "Parallel two-tower DMA-overlay implementation failed: $implementation_status"
}

open_run impl_1

set timing_report \
    [file join $report_dir poly_mul4096_two_tower_parallel_dma_timing.rpt]

set utilization_report \
    [file join $report_dir poly_mul4096_two_tower_parallel_dma_utilization.rpt]
...
set bit_destination \
    [file join \
        $deploy_dir \
        poly_mul4096_two_tower_parallel_dma.bit \
    ]

set hwh_destination \
    [file join \
        $deploy_dir \
        poly_mul4096_two_tower_parallel_dma.hwh \
    ]
...
set xsa_destination \
    [file join \
        $deploy_dir \
        poly_mul4096_two_tower_parallel_dma.xsa \
    ]
...
puts $manifest_handle "overlay=poly_mul4096_two_tower_parallel_dma"
...
puts $manifest_handle "coefficient_banks=4"
puts $manifest_handle "profile_banks=8"
puts $manifest_handle "parallel_core_cycles=1339394"
puts $manifest_handle "parallel_core_latency_us=13393.94"
...
puts $manifest_handle "expected_overlay_ramb36=50"
...
puts ""
puts "============================================================"
puts "PYNQ-Z2 PARALLEL TWO-TOWER DMA OVERLAY COMPLETE"
puts "============================================================"
