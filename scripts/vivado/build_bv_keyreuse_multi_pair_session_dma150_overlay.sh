...
required=(
  ...
  "$ROOT/rtl/evalmul3_bv_keyreuse_multi_pair_session_dma_wrapper.v"
...
echo
if [[ -f "$summary" ]]; then tr -d '
' <"$summary"; else echo "error: Vivado did not produce $summary" >&2; status=1; fi
marker='PASS: persistent-output multi-pair session dual-clock DMA overlay routed'
if [[ ! -f "$summary" ]] || ! tr -d '
