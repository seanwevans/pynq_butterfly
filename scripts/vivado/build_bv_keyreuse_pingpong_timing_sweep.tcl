# Timing-recovery sweep for the exact 160-DSP ping-pong coefficient-major
# OpenFHE BV evaluation-key-reuse core.
#
# Synthesizes once, then implements the same post-synthesis checkpoint with
# several combinations of the already-proven Vivado 2024.1 directives.

if {[llength $argv] != 2} {
    puts stderr \
        "usage: build_bv_keyreuse_pingpong_timing_sweep.tcl REPO_ROOT BUILD_ROOT"
...
summary_puts \
    $summary_file \
    "BV_KEYREUSE_PINGPONG_TIMING_SWEEP_BEGIN"
...
summary_puts \
    $summary_file \
    "baseline_route_wns_ns=-0.443"

summary_puts \
    $summary_file \
    "baseline_post_phys_wns_ns=-0.160"

summary_puts \
    $summary_file \
    "baseline_post_phys_failing_paths=31"
...
summary_puts \
    $summary_file \
    "BV_KEYREUSE_PINGPONG_TIMING_SWEEP_SYNTH_DSP48E1=$synth_dsp"
...
#
# Every directive below has already executed successfully in the preceding
# coefficient-major physical experiments.
set strategies [list \
    [list \
        ExploreWithRemap \
        ExploreWithRemap \
...
summary_puts \
    $summary_file \
    "BV_KEYREUSE_PINGPONG_TIMING_SWEEP_BEST_STRATEGY=$best_strategy"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_PINGPONG_TIMING_SWEEP_BEST_STAGE=$best_stage"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_PINGPONG_TIMING_SWEEP_WNS_NS=$best_wns"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_PINGPONG_TIMING_SWEEP_FAILING_PATHS=$best_failing"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_PINGPONG_TIMING_SWEEP_LUTS=$luts"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_PINGPONG_TIMING_SWEEP_REGISTERS=$regs"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_PINGPONG_TIMING_SWEEP_DSP48E1=$dsp"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_PINGPONG_TIMING_SWEEP_RAMB18E1=$ram18"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_PINGPONG_TIMING_SWEEP_RAMB36E1=$ram36"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_PINGPONG_TIMING_SWEEP_SRL=$srl"
...
if {$dsp != $expected_dsp_count} {
    summary_puts \
        $summary_file \
        "FAIL: best ping-pong implementation does not contain exactly 160 DSP48E1"
...
if {$best_wns < 0.000} {
    summary_puts \
        $summary_file \
        "FAIL: all ping-pong timing-recovery strategies missed 100 MHz"
...
summary_puts \
    $summary_file \
    "BV_KEYREUSE_PINGPONG_TIMING_SWEEP_END"
