...
WORK_DIR="${BV_KEYREUSE_COEFFICIENT_MAJOR_OOC_WORK:-/mnt/f/v/bv_keyreuse_coefficient_major_ooc}"
...
required=(
  ...
  "$ROOT/scripts/vivado/build_bv_keyreuse_coefficient_major_ooc.tcl"
)
...
tcl_windows="$repo_windows\\scripts\\vivado\\build_bv_keyreuse_coefficient_major_ooc.tcl"

cmd_file="$WORK_DIR/run_bv_keyreuse_coefficient_major_ooc.cmd"
...
marker='PASS: coefficient-major evaluation-key-reuse core routed at 100 MHz'

if [[ ! -f "$summary" ]] \
  || ! tr -d '\r' <"$summary" | grep -Fxq "$marker"; then
  echo "error: Vivado did not record the coefficient-major OOC PASS marker" >&2
  status=1
fi
