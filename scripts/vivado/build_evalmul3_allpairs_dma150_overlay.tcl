# Build a complete PYNQ-Z2 AXI-DMA overlay for the fused two-tower
# evaluation-domain ciphertext multiplier.
#
# Usage:
#   vivado -mode batch -source build_evalmul3_allpairs_dma150_overlay.tcl \
#     -tclargs REPO_ROOT BUILD_ROOT DEPLOY_DIR

if {[llength $argv] != 3} {
    puts stderr \
        "usage: build_evalmul3_allpairs_dma150_overlay.tcl REPO_ROOT BUILD_ROOT DEPLOY_DIR"
    exit 2
}
...
set project_name "evalmul3_allpairs_dma150"
set bd_name "d"
...
summary_puts $summary_file "EVALMUL3_ALLPAIRS_BUILD_BEGIN"
...
set rtl_files [list \
    ...
    [file join \
        $repo_root \
        "rtl" \
        "evalmul3_all_tower_axis_core.sv"] \
    [file join \
        $repo_root \
        "rtl" \
        "evalmul3_two_tower_axis_dma_wrapper.v"] \
]
...
set accel [
    create_bd_cell \
        -type module \
        -reference evalmul3_two_tower_axis_dma_wrapper \
        evalmul3
]
...
connect_bd_intf_net \
    ...
    [get_bd_intf_pins evalmul3/S_AXIS]

connect_bd_intf_net \
    [get_bd_intf_pins evalmul3/M_AXIS] \
...
summary_puts $summary_file \
    "EVALMUL3_ALLPAIRS_WNS_NS=$wns"

summary_puts $summary_file \
    "EVALMUL3_ALLPAIRS_FAILING_PATHS=[llength $failing_paths]"

summary_puts $summary_file \
    "EVALMUL3_ALLPAIRS_LUTS=$lut_count"

summary_puts $summary_file \
    "EVALMUL3_ALLPAIRS_REGISTERS=$register_count"

summary_puts $summary_file \
    "EVALMUL3_ALLPAIRS_DSP48E1=$dsp_count"

summary_puts $summary_file \
    "EVALMUL3_ALLPAIRS_RAMB18E1=$ramb18_count"

summary_puts $summary_file \
    "EVALMUL3_ALLPAIRS_RAMB36E1=$ramb36_count"
...
if {$wns < 0.000} {
    ...
    summary_puts $summary_file \
        "EVALMUL3_ALLPAIRS_WORST_PATH_GROUP=$worst_path_group"

    summary_puts $summary_file \
        "FAIL: all-pair overlay missed routed timing"
...
write_hw_platform \
    ...
    [file join \
        $build_root \
        "evalmul3_allpairs_dma150_platform.xsa"]
