# Timing-recovery sweep for the exact 160-DSP coefficient-major
# OpenFHE BV evaluation-key-reuse core.
#
# The sweep synthesizes once, then implements the same post-synthesis
# checkpoint with three previously proven strategy combinations.

if {[llength $argv] != 2} {
    puts stderr \
        "usage: build_bv_keyreuse_160dsp_timing_sweep.tcl REPO_ROOT BUILD_ROOT"
...
summary_puts \
    $summary_file \
    "BV_KEYREUSE_160DSP_TIMING_SWEEP_BEGIN"
...
summary_puts \
    $summary_file \
    "baseline_best_wns_ns=-0.121"

summary_puts \
    $summary_file \
    "baseline_failing_paths=20"
...
summary_puts \
    $summary_file \
    "BV_KEYREUSE_160DSP_TIMING_SWEEP_SYNTH_DSP48E1=$synth_dsp"
...
#
# These are the same strategy families that previously recovered the
# 192-DSP EvalMul3/BV-MAC co-residency design from negative slack.
set strategies [list \
    [list \
...
summary_puts \
    $summary_file \
    "BV_KEYREUSE_160DSP_TIMING_SWEEP_BEST_STRATEGY=$best_strategy"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_160DSP_TIMING_SWEEP_BEST_STAGE=$best_stage"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_160DSP_TIMING_SWEEP_WNS_NS=$best_wns"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_160DSP_TIMING_SWEEP_FAILING_PATHS=$best_failing"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_160DSP_TIMING_SWEEP_LUTS=$luts"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_160DSP_TIMING_SWEEP_REGISTERS=$regs"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_160DSP_TIMING_SWEEP_DSP48E1=$dsp"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_160DSP_TIMING_SWEEP_RAMB18E1=$ram18"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_160DSP_TIMING_SWEEP_RAMB36E1=$ram36"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_160DSP_TIMING_SWEEP_SRL=$srl"
...
if {$dsp != $expected_dsp_count} {
    summary_puts \
        $summary_file \
        "FAIL: best implementation does not contain exactly 160 DSP48E1"
...
if {$best_wns < 0.000} {
    summary_puts \
        $summary_file \
        "FAIL: all coefficient-major timing-recovery strategies missed 100 MHz"
...
summary_puts \
    $summary_file \
    "BV_KEYREUSE_160DSP_TIMING_SWEEP_END"
