# Build a complete PYNQ-Z2 AXI-DMA overlay for the exact
# persistent-output multi-pair OpenFHE BV evaluation-key-reuse session.
#
# Usage:
#   vivado -mode batch -source build_bv_keyreuse_multi_pair_session_dma150_overlay.tcl \
#     -tclargs REPO_ROOT BUILD_ROOT DEPLOY_DIR

if {[llength $argv] != 3} {
    puts stderr \
        "usage: build_bv_keyreuse_multi_pair_session_dma150_overlay.tcl REPO_ROOT BUILD_ROOT DEPLOY_DIR"
    exit 2
}
...
set project_name "evalmul3_bv_keyreuse_multi_pair_session_dma150"
set bd_name "d"
...
summary_puts $summary_file "BV_KEYREUSE_MULTI_PAIR_SESSION_DMA150_BUILD_BEGIN"
...
set rtl_files [list \
    ...
    [file join \
        $repo_root \
        "rtl" \
        "evalmul3_bv_keyreuse_multi_pair_session_dma_wrapper.v"] \
]
...
set accel [
    create_bd_cell \
        -type module \
        -reference evalmul3_bv_keyreuse_multi_pair_session_dma_wrapper \
        multi_pair
]
...
connect_bd_intf_net \
    ...
    [get_bd_intf_pins multi_pair/S_AXIS]

connect_bd_intf_net \
    [get_bd_intf_pins multi_pair/M_AXIS] \
...
connect_bd_net \
    ...
    [get_bd_pins multi_pair/aresetn]
...
summary_puts $summary_file \
    "BV_KEYREUSE_MULTI_PAIR_SESSION_DMA150_WNS_NS=$wns"

summary_puts $summary_file \
    "BV_KEYREUSE_MULTI_PAIR_SESSION_DMA150_FAILING_PATHS=[llength $failing_paths]"

summary_puts $summary_file \
    "BV_KEYREUSE_MULTI_PAIR_SESSION_DMA150_LUTS=$lut_count"

summary_puts $summary_file \
    "BV_KEYREUSE_MULTI_PAIR_SESSION_DMA150_REGISTERS=$register_count"

summary_puts $summary_file \
    "BV_KEYREUSE_MULTI_PAIR_SESSION_DMA150_DSP48E1=$dsp_count"

summary_puts $summary_file \
    "BV_KEYREUSE_MULTI_PAIR_SESSION_DMA150_RAMB18E1=$ramb18_count"

summary_puts $summary_file \
    "BV_KEYREUSE_MULTI_PAIR_SESSION_DMA150_RAMB36E1=$ramb36_count"
...
summary_puts $summary_file \
    "BV_KEYREUSE_MULTI_PAIR_SESSION_DMA150_SRL=$srl_count"

if {$dsp_count != 160} {
    summary_puts $summary_file \
        "FAIL: expected exactly 160 DSP48E1 in the drain-overlap overlay"
...
if {$wns < 0.000} {
    summary_puts $summary_file \
        "FAIL: persistent-output multi-pair session overlay missed routed timing"
...
set deploy_stem [
    file join \
        $deploy_dir \
        "evalmul3_bv_keyreuse_multi_pair_session_dma150"
]
...
write_hw_platform \
    ...
    [file join \
        $build_root \
        "evalmul3_bv_keyreuse_multi_pair_session_dma150_platform.xsa"]
...
summary_puts $summary_file \
    "BV_KEYREUSE_MULTI_PAIR_SESSION_DMA150_BUILD_END"
