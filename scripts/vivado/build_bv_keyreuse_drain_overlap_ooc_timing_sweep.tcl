# Timing sweep for the exact 160-DSP coefficient-major compute-drain-overlap
# OpenFHE BV evaluation-key-reuse core.
#
# Synthesizes once, then implements the same post-synthesis checkpoint with
# five Vivado 2024.1 directive combinations already used successfully in this
# project.

if {[llength $argv] != 2} {
    puts stderr \
        "usage: build_bv_keyreuse_drain_overlap_ooc_timing_sweep.tcl REPO_ROOT BUILD_ROOT"
...
set top "evalmul3_bv_keyreuse_drain_overlap_axis_core"
...
summary_puts \
    $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_OOC_TIMING_SWEEP_BEGIN"
...
summary_puts \
    $summary_file \
    "eval_metadata_bank_tagged=1"

summary_puts \
    $summary_file \
    "bv_metadata_bank_tagged=1"

summary_puts \
    $summary_file \
    "per_bank_completion_counters=1"

summary_puts \
    $summary_file \
    "wait_compute_state_removed=1"
...
summary_puts \
    $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_OOC_SYNTH_DSP48E1=$synth_dsp"
...
summary_puts \
    $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_OOC_BEST_STRATEGY=$best_strategy"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_OOC_BEST_STAGE=$best_stage"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_OOC_WNS_NS=$best_wns"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_OOC_FAILING_PATHS=$best_failing"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_OOC_LUTS=$luts"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_OOC_REGISTERS=$regs"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_OOC_DSP48E1=$dsp"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_OOC_RAMB18E1=$ram18"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_OOC_RAMB36E1=$ram36"

summary_puts \
    $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_OOC_SRL=$srl"
...
if {$dsp != $expected_dsp_count} {
    summary_puts \
        $summary_file \
        "FAIL: best drain-overlap implementation does not contain exactly 160 DSP48E1"
...
if {$best_wns < 0.000} {
    summary_puts \
        $summary_file \
        "FAIL: all drain-overlap timing strategies missed 100 MHz"
...
summary_puts \
    $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_OOC_TIMING_SWEEP_END"
