# Build a complete PYNQ-Z2 AXI-DMA overlay for exact coefficient-major compute-drain overlap
# OpenFHE BV evaluation-key reuse.
#
# Usage:
#   vivado -mode batch -source build_bv_keyreuse_drain_overlap_dma150_overlay.tcl \
#     -tclargs REPO_ROOT BUILD_ROOT DEPLOY_DIR

if {[llength $argv] != 3} {
    puts stderr \
        "usage: build_bv_keyreuse_drain_overlap_dma150_overlay.tcl REPO_ROOT BUILD_ROOT DEPLOY_DIR"
    exit 2
}
...
set project_name "evalmul3_bv_keyreuse_drain_overlap_dma150"
set bd_name "d"
...
summary_puts $summary_file "BV_KEYREUSE_DRAIN_OVERLAP_DMA150_BUILD_BEGIN"
...
set rtl_files [list \
    ...
    [file join \
        $repo_root \
        "rtl" \
        "evalmul3_bv_keyreuse_drain_overlap_dma_wrapper.v"] \
]
...
set accel [
    create_bd_cell \
        -type module \
        -reference evalmul3_bv_keyreuse_drain_overlap_dma_wrapper \
        drain_overlap
]
...
connect_bd_intf_net \
    ...
    [get_bd_intf_pins drain_overlap/S_AXIS]

connect_bd_intf_net \
    [get_bd_intf_pins drain_overlap/M_AXIS] \
...
connect_bd_net \
    ...
    [get_bd_pins drain_overlap/aclk]
...
connect_bd_net \
    ...
    [get_bd_pins drain_overlap/aresetn]
...
summary_puts $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_DMA150_WNS_NS=$wns"

summary_puts $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_DMA150_FAILING_PATHS=[llength $failing_paths]"

summary_puts $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_DMA150_LUTS=$lut_count"

summary_puts $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_DMA150_REGISTERS=$register_count"

summary_puts $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_DMA150_DSP48E1=$dsp_count"

summary_puts $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_DMA150_RAMB18E1=$ramb18_count"

summary_puts $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_DMA150_RAMB36E1=$ramb36_count"
...
summary_puts $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_DMA150_SRL=$srl_count"
...
if {$wns < 0.000} {
    summary_puts $summary_file \
        "FAIL: coefficient-major compute-drain-overlap overlay missed routed timing"
...
set deploy_stem [
    file join \
        $deploy_dir \
        "evalmul3_bv_keyreuse_drain_overlap_dma150"
]
...
write_hw_platform \
    ...
    [file join \
        $build_root \
        "evalmul3_bv_keyreuse_drain_overlap_dma150_platform.xsa"]
...
summary_puts $summary_file \
    "BV_KEYREUSE_DRAIN_OVERLAP_DMA150_BUILD_END"
