...
WORK_DIR="${EVALMUL3_BV_RELINEARIZED_OOC_WORK:-/mnt/f/v/evalmul3_bv_relinearized_ooc}"
...
for path in \
  ...
  "$ROOT/scripts/vivado/build_evalmul3_bv_relinearized_ooc.tcl"
do
  [[ -f "$path" ]] || {
...
tcl_windows="$repo_windows\\scripts\\vivado\\build_evalmul3_bv_relinearized_ooc.tcl"

cmd_file="$WORK_DIR/run_evalmul3_bv_relinearized_ooc.cmd"
...
marker='PASS: exact fused EvalMul3 plus BV relinearization routed at 100 MHz'

if [[ ! -f "$summary" ]] \
  || ! tr -d '\r' <"$summary" | grep -Fxq "$marker"; then
  echo "error: Vivado did not record the fused OOC PASS marker" >&2
  status=1
fi
