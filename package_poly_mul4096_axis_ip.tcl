...
set ip_root [file join $root_dir ip poly_mul4096_axis_1_0]
set stage_dir [file join $ip_root staged_sources]
set project_dir [file join $root_dir vivado package_poly_mul4096_axis]
...
set rtl_names [list \
    ...
    ntt4096_twiddle_rom.sv \
    ntt4096_factor_rom.sv \
    ntt4096_coeff_bram.sv \
    ntt4096_cyclic_core.sv \
    forward_ntt4096_core.sv \
    inverse_ntt4096_core.sv \
    pointwise_mul4096_core.sv \
    poly_mul4096_core.sv \
    poly_mul4096_axis_core.sv \
]

set memory_names [list \
...
foreach name $rtl_names {
    ...
    set input_handle [open $source r]
    set text [read $input_handle]
    close $input_handle

    # Packaged IP must not depend on the repository's model directory.
    ...
    set output_handle [open $destination w]
    puts -nonewline $output_handle $text
    close $output_handle
}

foreach name $memory_names {
    set source [file join $model_dir $name]
    require_file $source
...
foreach name $memory_names {
    lappend staged_memory_files \
        [file join $stage_dir $name]
}

add_files -norecurse $staged_rtl_files
...
add_files -norecurse $staged_memory_files

foreach path $staged_memory_files {
    catch {
        set_property \
            file_type \
...
set_property name poly_mul4096_axis $core
set_property display_name {N=4096 Negacyclic Polynomial Multiplier AXI4-Stream} $core
set_property description \
    {Constant-time N=4096 negacyclic polynomial multiplier over q=1073692673 with AXI4-Stream input and output.} \
    $core
...
puts ""
puts "============================================================"
puts "N=4096 AXI4-STREAM IP PACKAGING COMPLETE"
puts "============================================================"
puts "IP root:       [file normalize $ip_root]"
puts "VLNV:          user.org:user:poly_mul4096_axis:1.0"
puts "Integrity:     $integrity_result"
puts "Input frame:   8192 x 32-bit words"
puts "Output frame:  4096 x 32-bit words"
puts "============================================================"
puts ""
