...
WORK_DIR="${EVALMUL3_BV_MAC_CORESIDENCY_WORK:-/mnt/f/v/evalmul3_bv_mac_coresidency}"
...
required=(
  ...
  "$ROOT/scripts/vivado/build_evalmul3_bv_mac_coresidency_ooc.tcl"
)
...
tcl_windows="$repo_windows\\scripts\\vivado\\build_evalmul3_bv_mac_coresidency_ooc.tcl"

cmd_file="$WORK_DIR/run_evalmul3_bv_mac_coresidency.cmd"
...
pass_marker='PASS: exact EvalMul3 and BV-MAC arithmetic cores co-routed at 100 MHz'

if [[ ! -f "$summary" ]] \
  || ! tr -d '\r' <"$summary" | grep -Fxq "$pass_marker"; then
  echo "error: Vivado did not record the co-residency PASS marker" >&2
  status=1
fi
