...
WORK_DIR="${EVALMUL3_BV_RELINEARIZED_DMA150_WORK:-/mnt/f/v/evalmul3_bv_relinearized_dma150_overlay}"
...
required=(
  ...
  "$ROOT/rtl/evalmul3_bv_relinearized_axis_dma_wrapper.v"
  "$ROOT/scripts/vivado/build_evalmul3_bv_relinearized_dma150_overlay.tcl"
)
...
tcl_windows="$repo_windows\\scripts\\vivado\\build_evalmul3_bv_relinearized_dma150_overlay.tcl"

cmd_file="$WORK_DIR/run_evalmul3_bv_relinearized_dma150_overlay.cmd"
...
marker='PASS: exact fused BV-relinearization dual-clock DMA overlay routed'

if [[ ! -f "$summary" ]] \
  || ! tr -d '\r' <"$summary" | grep -Fxq "$marker"; then
  echo "error: Vivado did not record the fused overlay PASS marker" >&2
  status=1
fi
