...
WORK_DIR="${BV_KEYREUSE_DRAIN_OVERLAP_DMA150_WORK:-/mnt/f/v/bv_keyreuse_drain_overlap_dma150_overlay}"
DEPLOY_DIR="$ROOT/deploy/bv_keyreuse_drain_overlap_dma150"
...
required=(
  ...
  "$ROOT/rtl/evalmul3_bv_keyreuse_drain_overlap_dma_wrapper.v"
  "$ROOT/scripts/vivado/build_bv_keyreuse_drain_overlap_dma150_overlay.tcl"
)
...
rm -f \
  "$DEPLOY_DIR/evalmul3_bv_keyreuse_drain_overlap_dma150.bit" \
  "$DEPLOY_DIR/evalmul3_bv_keyreuse_drain_overlap_dma150.hwh" \
...
tcl_windows="$repo_windows\\scripts\\vivado\\build_bv_keyreuse_drain_overlap_dma150_overlay.tcl"

cmd_file="$WORK_DIR/run_bv_keyreuse_drain_overlap_dma150_overlay.cmd"
...
marker='PASS: coefficient-major compute-drain-overlap dual-clock DMA overlay routed'

if [[ ! -f "$summary" ]] \
  || ! tr -d '\r' <"$summary" | grep -Fxq "$marker"; then
  echo "error: Vivado did not record the drain-overlap overlay PASS marker" >&2
  status=1
fi
