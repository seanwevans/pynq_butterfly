...
WORK_DIR="${BV_KEYREUSE_MULTI_PAIR_SESSION_OOC_WORK:-/mnt/f/v/bv_keyreuse_multi_pair_session_ooc_timing_sweep}"
...
required=(
  ...
  "$ROOT/scripts/vivado/build_bv_keyreuse_multi_pair_session_ooc_timing_sweep.tcl"
)
...
tcl_windows="$repo_windows\\scripts\\vivado\\build_bv_keyreuse_multi_pair_session_ooc_timing_sweep.tcl"

cmd_file="$WORK_DIR/run_bv_keyreuse_multi_pair_session_ooc_timing_sweep.cmd"
...
marker='PASS: persistent-output multi-pair session core routed at 100 MHz'

if [[ ! -f "$summary" ]] \
  || ! tr -d '\r' <"$summary" | grep -Fxq "$marker"; then
  echo "error: Vivado did not record the multi-pair OOC PASS marker" >&2
  status=1
fi
