...
set work_dir [file join $root_dir vivado package_poly_mul256_axis_batch]
set project_dir [file join $work_dir project]
set ip_dir [file join $root_dir ip poly_mul256_axis_batch_1_0]
set report_dir [file join $root_dir reports poly_mul256_axis_batch]
set constraint_file [file join $work_dir poly_mul256_axis_batch_clock.xdc]
...
proc map_interface_port {interface logical_name physical_name} {
...
puts ""
puts "============================================================"
puts "POLY_MUL256 BATCHED AXI-STREAM PACKAGE"
puts "============================================================"
puts "Root:    $root_dir"
puts "IP:      $ip_dir"
...
create_project \
    -force \
    package_poly_mul256_axis_batch \
    $project_dir \
...
set rtl_files [list \
    ...
    [file join $rtl_dir poly_mul256_axis_batch_core.sv] \
]
...
foreach source_file $rtl_files {
    set_property file_type SystemVerilog [get_files [file normalize $source_file]]
}

set_property top poly_mul256_axis_batch_core [get_filesets sources_1]

set constraint_handle [open $constraint_file w]
...
set_property \
    display_name \
    {OpenFHE N=256 Batched AXI-Stream Polynomial Multiplier} \
    $core

set_property \
    description \
    {DMA-ready batched AXI4-Stream adapter for the constant-time BRAM-backed OpenFHE-profile N=256 negacyclic polynomial multiplier.} \
    $core

# Replace any automatically inferred bus interfaces with explicit maps.
foreach interface [ipx::get_bus_interfaces -of_objects $core] {
    ipx::remove_bus_interface [get_property NAME $interface] $core
}

set s_axis [ipx::add_bus_interface S_AXIS $core]
...
map_interface_port $s_axis TDATA s_axis_tdata
map_interface_port $s_axis TKEEP s_axis_tkeep
map_interface_port $s_axis TVALID s_axis_tvalid
map_interface_port $s_axis TREADY s_axis_tready
map_interface_port $s_axis TLAST s_axis_tlast

set m_axis [ipx::add_bus_interface M_AXIS $core]
...
map_interface_port $m_axis TDATA m_axis_tdata
map_interface_port $m_axis TKEEP m_axis_tkeep
map_interface_port $m_axis TVALID m_axis_tvalid
map_interface_port $m_axis TREADY m_axis_tready
map_interface_port $m_axis TLAST m_axis_tlast

set clock_interface [ipx::add_bus_interface ACLK $core]
...
set_property interface_mode slave $clock_interface

map_interface_port $clock_interface CLK aclk

set associated_busif [ipx::add_bus_parameter ASSOCIATED_BUSIF $clock_interface]
...
set clock_frequency [ipx::add_bus_parameter FREQ_HZ $clock_interface]
set_property value 100000000 $clock_frequency

set reset_interface [ipx::add_bus_interface ARESETN $core]
...
set_property interface_mode slave $reset_interface

map_interface_port $reset_interface RST aresetn
...
set utilization_report [file join $report_dir poly_mul256_axis_batch_utilization.rpt]
set timing_report [file join $report_dir poly_mul256_axis_batch_timing.rpt]

report_utilization -file $utilization_report
report_timing_summary -file $timing_report

set ramb18_count [llength [get_cells -hierarchical -quiet -filter {REF_NAME == RAMB18E1}]]
set ramb36_count [llength [get_cells -hierarchical -quiet -filter {REF_NAME == RAMB36E1}]]
set dsp_count [llength [get_cells -hierarchical -quiet -filter {REF_NAME == DSP48E1}]]

puts ""
puts "============================================================"
puts "BATCHED AXI-STREAM PACKAGE COMPLETE"
puts "============================================================"
puts "VLNV: user.org:user:poly_mul256_axis_batch:1.0"
puts "Default batch products: 32"
