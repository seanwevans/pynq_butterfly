...
required=(
  ...
  "$ROOT/run_openfhe_multitower_board.sh"
...
ssh "$PYNQ_SSH" "rm -rf '$REMOTE_DIR' && mkdir -p '$REMOTE_DIR'"
...
scp -r "$VECTOR_DIR" "$PYNQ_SSH:$REMOTE_DIR/vectors"

ssh "$PYNQ_SSH" \
  "chmod +x '$REMOTE_DIR/run_openfhe_multitower_board.sh' '$REMOTE_DIR/run_fpga_multitower_buffered.py'"
