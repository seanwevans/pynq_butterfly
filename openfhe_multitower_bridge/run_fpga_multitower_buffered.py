...
def build_profile_lane(
    ...
    if modulus >= 1 << 30:
        raise ValueError(f"{profile_directory}: modulus is not below 2^30")
    ...
    if payload.size != 16_382:
        raise RuntimeError(
            f"Profile payload contains {payload.size} words; expected 16382"
        )

    return np.uint32(modulus), np.uint32(reciprocal), payload


def build_profile_frame(profile0: Path, profile1: Path) -> np.ndarray:
...
def build_batch_frame(
    ...
    expected_dma_words = product_count * 2 * N
    if dma0.size != expected_dma_words or dma1.size != expected_dma_words:
        raise ValueError(
            "DMA tower input length does not match product_count * 8192"
        )
    ...
    lane0[1] = np.uint32(product_count)
    lane1[1] = np.uint32(product_count)
    lane0[2:] = dma0
    lane1[2:] = dma1
...
def require_equal(
    ...
    raise AssertionError(
        f"{label} mismatch at product {product}, coefficient {coefficient}: "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Run an arbitrary-tower OpenFHE DCRTPoly multiplication batch "
    ...
    if tower_count < 1 or product_count < 1:
        raise ValueError("tower_count and product_count must be positive")
    if ring_dimension != N:
        raise ValueError(f"This overlay requires N={N}; metadata uses {ring_dimension}")

    pair_count = (tower_count + 1) // 2
    expected_tower_words = product_count * N
    expected_dma_words = product_count * 2 * N
    ...
    if xsa_path.exists():
        raise RuntimeError(
            f"Rename or remove {xsa_path}; PYNQ may select it instead of the HWH"
        )
    ...
    print(
        f"DCRT batch: towers={tower_count}, pairs={pair_count}, "
        f"products={product_count}"
    )
    print(
        "pair  towers       profile_us  median_us  us/DCRT  "
        "pair-products/s"
    )
    ...
    for pair_index in range(pair_count):
        ...
        key, profile_frame = frame_cache.get(profile0, profile1)
        ...
        expected0 = read_u32le(
            vector_root / "towers" / name0 / "openfhe_expected.bin",
            expected_tower_words,
        )
        expected1 = read_u32le(
            vector_root / "towers" / name1 / "openfhe_expected.bin",
            expected_tower_words,
        )
        ...
        frame = build_batch_frame(dma0, dma1, product_count)
        ...
        try:
            ...
            warmup0, warmup1 = split_results(receive_buffer)
            require_equal(warmup0, expected0, f"pair {pair_index} warmup lane0")
            require_equal(warmup1, expected1, f"pair {pair_index} warmup lane1")
            ...
            tower_products_verified += product_count
            ...
            pair_product_rate = product_count * 1_000_000.0 / median_us
            towers_label = f"{tower0},{tower1}" if tower1_real else f"{tower0},dup"

            print(
                f"{pair_index:4d}  {towers_label:10s}  "
                f"{profile_us:10.2f}  {median_us:9.2f}  "
                f"{median_us / product_count:7.2f}  "
                f"{pair_product_rate:15.2f}"
            )
            print(
                f"      PASS exact; warmup={warmup_us:.2f} us; "
                f"runs={','.join(f'{value:.2f}' for value in timings_us)}"
            )
    ...
    compute_dcrt_us = pair_median_us_total / product_count
    end_to_end_dcrt_us = (
        pair_median_us_total + profile_load_us_total
    ) / product_count
    compute_dcrt_rate = product_count * 1_000_000.0 / pair_median_us_total
    end_to_end_dcrt_rate = (
        product_count * 1_000_000.0
        / (pair_median_us_total + profile_load_us_total)
    )
    tower_pair_rate = (
        pair_count * product_count * 1_000_000.0 / pair_median_us_total
    )
    theoretical_pair_us = (
        BUFFERED_STEADY_CYCLES_PER_PRODUCT / CLOCK_HZ * 1_000_000.0
    )

    summary = {
        "format": "openfhe-dcrtpoly-multitower-fpga-results-v1",
        ...
        "compute_us_per_dcrt_product": compute_dcrt_us,
        "end_to_end_us_per_dcrt_product": end_to_end_dcrt_us,
        "compute_dcrt_products_per_second": compute_dcrt_rate,
        "end_to_end_dcrt_products_per_second": end_to_end_dcrt_rate,
        "effective_tower_pair_products_per_second": tower_pair_rate,
        "theoretical_pair_us_per_product": theoretical_pair_us,
        "verified_tower_products": tower_products_verified,
    }
    ...
    print()
    print("PASS: every FPGA tower product matches OpenFHE")
    print(f"verified_tower_products={tower_products_verified}")
    print(f"compute_us_per_DCRTPoly={compute_dcrt_us:.2f}")
    print(f"compute_DCRTPoly_products_per_second={compute_dcrt_rate:.2f}")
    print(f"end_to_end_us_per_DCRTPoly={end_to_end_dcrt_us:.2f}")
    print(
        "end_to_end_DCRTPoly_products_per_second="
        f"{end_to_end_dcrt_rate:.2f}"
    )
    print(f"effective_tower_pair_products_per_second={tower_pair_rate:.2f}")
