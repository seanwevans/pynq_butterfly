...
EXPECTED_CORE_CYCLES = overlay_constants.DUAL_BUTTERFLY_CORE_CYCLES
EXPECTED_CORE_LATENCY_US = 6_318.10

PREVIOUS_PHYSICAL_MEDIAN_US = 14_384.08
...
def main() -> None:
    ...
    try:
        ...
        print(
            "PASS: first timing-isolated product matches OpenFHE"
        )
        ...
        for index in range(args.timed_runs):
            ...
            validate(
                final0,
                expected0,
                f"Timed tower 0 run {index}",
            )

            validate(
                final1,
                expected1,
                f"Timed tower 1 run {index}",
            )
        ...
        print(
            f"PASS: {args.timed_runs} products all match OpenFHE"
        )

        print(
            f"Minimum: {min(timings):.2f} us"
        )
        ...
        print(
            f"Maximum: {max(timings):.2f} us"
        )

        print(
            f"Two-tower DCRTPoly products/s: "
            f"{1_000_000.0 / median_us:.2f}"
        )

        print(
            f"Individual tower products/s: "
            f"{2_000_000.0 / median_us:.2f}"
        )

        print(
            f"Arithmetic ceiling: "
            f"{ceiling:.2f} DCRTPoly products/s"
        )
        ...
        print(
            f"Core latency: "
            f"{EXPECTED_CORE_LATENCY_US:.2f} us"
        )

        print(
            f"Median DMA/software overhead: "
            f"{overhead_us:.2f} us"
        )

        print(
            f"Physical speedup versus previous overlay: "
            f"{physical_speedup:.3f}x"
        )

        if median_us < args.openfhe_reference_us:
            print(
                "PASS: FPGA end-to-end median is "
                f"{cpu_ratio:.3f}x faster than the "
                "supplied OpenFHE reference"
            )
        else:
            print(
                "INFO: FPGA end-to-end median is "
                f"{1.0 / cpu_ratio:.3f}x slower than the "
                "supplied OpenFHE reference"
            )

        print(
            "PASS board timing-isolated two-tower OpenFHE stage"
        )
