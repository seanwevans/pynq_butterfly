...
EXPECTED_CORE_CYCLES = 1_376_268
EXPECTED_CORE_LATENCY_US = 13_762.68
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Validate and benchmark the PYNQ-Z2 N=4096 "
            "negacyclic polynomial-multiplier DMA overlay."
        )
    )
    ...
    parser.add_argument(
        "--overlay",
        type=Path,
        default=Path("poly_mul4096_dma.bit"),
    )
    ...
    try:
        ...
        if not np.array_equal(
            np.asarray(receive_buffer),
            expected,
        ):
            mismatch_indices = np.flatnonzero(
                np.asarray(receive_buffer) != expected
            )

            first_index = int(mismatch_indices[0])

            raise AssertionError(
                "First DMA result does not match the golden convolution: "
                f"index={first_index}, "
                f"result={int(receive_buffer[first_index])}, "
                f"expected={int(expected[first_index])}"
            )

        print(
            "PASS: first N=4096 DMA product matches golden convolution"
        )
        ...
        for run_index in range(args.timed_runs):
            ...
            if not np.array_equal(
                ...
                raise AssertionError(
                    f"Timed run {run_index} mismatch: "
                    f"index={first_index}, "
                    f"result={int(receive_buffer[first_index])}, "
                    f"expected={int(expected[first_index])}"
                )
        ...
        print(
            f"PASS: {args.timed_runs} timed DMA products all match"
        )
