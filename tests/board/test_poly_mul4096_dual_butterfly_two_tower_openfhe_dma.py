...
EXPECTED_CORE_CYCLES = 607_234
EXPECTED_CORE_LATENCY_US = 6_072.34

PREVIOUS_CORE_CYCLES = overlay_constants.SINGLE_BUTTERFLY_CORE_CYCLES
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "through the dual-butterfly PYNQ-Z2 overlay."
        )
    )
    ...
    parser.add_argument(
        ...
        default=7161.35,
        help=(
            "Measured OpenFHE software time used only "
    ...
    try:
        ...
        print(
            "PASS: first dual-butterfly two-tower product matches OpenFHE"
        )
        ...
        print(
            f"PASS: {args.timed_runs} dual-butterfly two-tower products all match OpenFHE"
        )
        ...
        print(
            f"Physical speedup versus previous parallel core: {physical_speedup:.3f}x"
        )

        print(
            f"OpenFHE reference: {args.openfhe_reference_us:.2f} us"
        )

        if median_us < args.openfhe_reference_us:
            print(
                f"PASS: FPGA end-to-end median is {cpu_ratio:.3f}x faster than the supplied OpenFHE reference"
            )
        else:
            print(
                f"INFO: FPGA end-to-end median is {1.0 / cpu_ratio:.3f}x slower than the supplied OpenFHE reference"
            )
        ...
        print(
            "PASS board dual-butterfly two-tower OpenFHE stage"
        )
