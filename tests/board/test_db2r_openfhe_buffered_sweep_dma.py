...
STEADY_STATE_CYCLES_PER_PRODUCT = overlay_constants.STEADY_STATE_CYCLES_PER_PRODUCT
CLOCK_MHZ = 100.0

DEFAULT_BATCH_COUNTS = (1, 2, 4, 8, 16)
DEFAULT_OPENFHE_REFERENCE_US = 7_161.35

PREVIOUS_PREFETCH_MEDIAN_US = {
    1: 7_198.26,
    2: 6_795.17,
    4: 6_589.16,
    8: 6_483.97,
    ...
    for product_index, (expected0, expected1) in enumerate(expected):
        ...
        for tower_index, actual, wanted in (
            ...
            raise AssertionError(
                ...
                f"tower {tower_index}, coefficient {mismatch}: "
...
def resolve_dma(overlay: Overlay):
    ...
    if len(names) != 1:
        raise RuntimeError(
            f"Expected exactly one DMA; found {names}"
        )
...
def percentile_nearest_rank(
    ...
    index = max(
        0,
        int(np.ceil(fraction * len(ordered))) - 1,
    )
...
def summarize(
    ...
    nonfinal_handoffs = max(batch_count - 1, 0)

    accelerator_model_cycles = (
        batch_count * PER_PRODUCT_CORE_CYCLES
        + nonfinal_handoffs
        * HANDOFF_CYCLES_PER_NONFINAL_PRODUCT
    )

    accelerator_model_us = (
        accelerator_model_cycles / CLOCK_MHZ
    )

    residual_batch_us = (
        median_batch_us - accelerator_model_us
    )
    ...
    previous_product_us = PREVIOUS_PREFETCH_MEDIAN_US.get(
        batch_count,
        float("nan"),
    )
    ...
    return {
        ...
        "input_words": (
            2 + batch_count * WORDS_PER_PRODUCT_INPUT
        ),
        "input_bytes": (
            8 * (2 + batch_count * WORDS_PER_PRODUCT_INPUT)
        ),
        "output_words": (
            batch_count * WORDS_PER_PRODUCT_OUTPUT
        ),
        "output_bytes": (
            8 * batch_count * WORDS_PER_PRODUCT_OUTPUT
        ),
        "minimum_batch_us": minimum_batch_us,
        ...
        "accelerator_model_cycles": accelerator_model_cycles,
        "accelerator_model_us": accelerator_model_us,
        "residual_batch_us": residual_batch_us,
        "residual_product_us": (
            residual_batch_us / batch_count
        ),
        "products_per_second": (
            1_000_000.0 / median_product_us
        ),
        "individual_towers_per_second": (
            2_000_000.0 / median_product_us
        ),
        "speedup_vs_openfhe": (
            openfhe_reference_us / median_product_us
        ),
        "previous_prefetch_product_us": previous_product_us,
        "speedup_vs_previous_prefetch": (
            previous_product_us / median_product_us
        ),
    }
...
def print_summary(
    ...
    count = int(summary["batch_count"])

    print()
    print("============================================================")
    print(f"BUFFERED-HANDOFF BATCH COUNT {count}")
    print("============================================================")
    print(
        f"Input:  {summary['input_words']} words, "
        f"{summary['input_bytes']} bytes"
    )
    print(
        f"Output: {summary['output_words']} words, "
        f"{summary['output_bytes']} bytes"
    )
    print(
        f"Minimum batch: {summary['minimum_batch_us']:.2f} us"
    )
    print(
        f"Median batch: {summary['median_batch_us']:.2f} us"
    )
    print(
        f"Mean batch: {summary['mean_batch_us']:.2f} us"
    )
    print(
        f"p95 batch: {summary['p95_batch_us']:.2f} us"
    )
    print(
        f"Maximum batch: {summary['maximum_batch_us']:.2f} us"
    )
    print(
        f"Median per DCRTPoly product: "
        f"{summary['median_product_us']:.2f} us"
    )
    print(
        f"Mean per DCRTPoly product: "
        f"{summary['mean_product_us']:.2f} us"
    )
    print(
        f"p95 per DCRTPoly product: "
        f"{summary['p95_product_us']:.2f} us"
    )
    print(
        f"Core plus nonfinal handoffs: "
        f"{summary['accelerator_model_us']:.2f} us"
    )
    print(
        f"Residual batch time: "
        f"{summary['residual_batch_us']:.2f} us"
    )
    print(
        f"Residual time per product: "
        f"{summary['residual_product_us']:.2f} us"
    )
    print(
        f"Measured DCRTPoly products/s: "
        f"{summary['products_per_second']:.2f}"
    )
    print(
        f"Speedup versus previous prefetch batch: "
        f"{summary['speedup_vs_previous_prefetch']:.3f}x"
    )

    if float(summary["median_product_us"]) < openfhe_reference_us:
        print(
            "PASS: buffered FPGA median is "
            f"{summary['speedup_vs_openfhe']:.3f}x "
            "faster than OpenFHE"
        )
    else:
        print(
            "INFO: buffered FPGA median is "
...
def main() -> None:
    ...
    if not batch_counts:
        raise ValueError(
            "At least one batch count is required"
        )

    if any(count <= 0 for count in batch_counts):
        raise ValueError(
            "Batch counts must be positive"
        )

    if args.timed_batches <= 0:
        raise ValueError(
            "--timed-batches must be positive"
        )
    ...
    print(
        "PASS: q0 and q1 profiles loaded once "
        "for the complete buffered-handoff sweep"
    )
    ...
    for batch_count in batch_counts:
        ...
        try:
            ...
            final0, final1 = validate_batch(
                receive_buffer,
                expected,
                f"Buffered batch {batch_count} first run",
            )

            print()
            print(
                f"PASS: buffered batch count {batch_count} "
                "first run exactly matches OpenFHE"
            )
            print(f"First batch: {first_us:.2f} us")
            print(
                f"First amortized product: "
                f"{first_us / batch_count:.2f} us"
            )
            ...
            batch_output = (
                output_root / f"batch{batch_count}"
            )
            ...
            for product_index in range(batch_count):
                start = (
                    product_index * WORDS_PER_PRODUCT_OUTPUT
                )

                stop = (
                    start + WORDS_PER_PRODUCT_OUTPUT
                )
            ...
            print(
                f"PASS: {args.timed_batches} timed "
                f"buffered batches of count {batch_count} "
                "all match OpenFHE"
            )
    ...
    csv_path = output_root / "buffered_sweep.csv"
    json_path = output_root / "buffered_sweep.json"
        ...
        json.dumps(
            {
                ...
                "timed_batches_per_count": (
                    args.timed_batches
                ),
                "openfhe_reference_us": (
                    args.openfhe_reference_us
                ),
                "per_product_core_cycles": (
                    PER_PRODUCT_CORE_CYCLES
                ),
                "handoff_cycles_per_nonfinal_product": (
                    HANDOFF_CYCLES_PER_NONFINAL_PRODUCT
    ...
    fixed_us, incremental_us = np.linalg.lstsq(
        design,
        medians,
        rcond=None,
    )[0]
    ...
    per_product = [
        float(summary["median_product_us"])
        for summary in summaries
    ]
    ...
    print()
    print("============================================================")
    print("BUFFERED-HANDOFF SWEEP COMPLETE")
    print("============================================================")
    print(
        f"Hardware steady-state spacing: "
        f"{STEADY_STATE_CYCLES_PER_PRODUCT} clocks"
    )
    print(
        f"Hardware steady-state latency: "
        f"{STEADY_STATE_CYCLES_PER_PRODUCT / CLOCK_MHZ:.2f} us"
    )
    print(
        f"Linear fit fixed batch cost: "
        f"{fixed_us:.2f} us"
    )
    print(
        f"Linear fit incremental cost/product: "
        f"{incremental_us:.2f} us"
    )
    print(
        f"Linear-fit asymptotic products/s: "
        f"{1_000_000.0 / incremental_us:.2f}"
    )
    print(
        f"Best measured batch count: "
        f"{best['batch_count']}"
    )
    print(
        f"Best measured median/product: "
        f"{best['median_product_us']:.2f} us"
    )
    print(
        f"Best measured OpenFHE speedup: "
        f"{best['speedup_vs_openfhe']:.3f}x"
    )
    print(
        f"Best speedup versus prefetch overlay: "
        f"{best['speedup_vs_previous_prefetch']:.3f}x"
    )

    if monotonic:
        print(
            "PASS: amortized median did not increase "
            "as buffered batch count grew"
        )
    else:
        print(
            "INFO: amortized medians contain at least one "
            "non-monotonic step; inspect the CSV"
        )

    if float(best["median_product_us"]) <= 6_375.0:
        print(
            "PASS: batch-16 buffered target of "
            "6375 us/product was reached"
        )
    else:
        print(
            "INFO: batch-16 buffered target of "
            "6375 us/product was not reached"
        )

    print(f"CSV:  {csv_path}")
    print(f"JSON: {json_path}")
    print(
        "PASS board buffered-handoff OpenFHE sweep"
    )
