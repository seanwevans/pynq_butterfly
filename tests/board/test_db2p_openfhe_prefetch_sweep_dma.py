...
TRANSITION_CYCLES_PER_NONFINAL_PRODUCT = 8_193
STEADY_STATE_CYCLES_PER_PRODUCT = 640_003
CLOCK_MHZ = 100.0

DEFAULT_BATCH_COUNTS = (1, 2, 4, 8, 16)
DEFAULT_OPENFHE_REFERENCE_US = 7_161.35

PREVIOUS_BATCH_MEDIAN_US = {
    1: 7_216.80,
    2: 6_860.69,
    4: 6_665.12,
    8: 6_567.20,
...
def build_batch_frame(
    ...
    input_words = (
        2
        + batch_count * WORDS_PER_PRODUCT_INPUT
    )
...
def validate_batch(
    ...
    for product_index, (expected0, expected1) in enumerate(expected):
        ...
        for tower_index, actual, wanted in (
            ...
            mismatch = int(
                np.flatnonzero(actual != wanted)[0]
            )
...
def summarize(
    ...
    transition_count = max(batch_count - 1, 0)

    accelerator_model_cycles = (
        batch_count * PER_PRODUCT_CORE_CYCLES
        + transition_count
        * TRANSITION_CYCLES_PER_NONFINAL_PRODUCT
    )
    ...
    median_product_us = (
        median_batch_us / batch_count
    )

    mean_product_us = (
        mean_batch_us / batch_count
    )

    p95_product_us = (
        p95_batch_us / batch_count
    )

    previous_product_us = PREVIOUS_BATCH_MEDIAN_US.get(
        batch_count,
        float("nan"),
    )
    ...
    return {
        ...
        "previous_batch_product_us": previous_product_us,
        "speedup_vs_previous_batch": (
...
def print_summary(
    ...
    print()
    print("============================================================")
    print(f"PREFETCH BATCH COUNT {count}")
    print("============================================================")
    print(
    ...
    print(
        f"Minimum batch: "
        f"{summary['minimum_batch_us']:.2f} us"
    )
    print(
        f"Median batch: "
        f"{summary['median_batch_us']:.2f} us"
    )
    print(
        f"Mean batch: "
        f"{summary['mean_batch_us']:.2f} us"
    )
    print(
        f"p95 batch: "
        f"{summary['p95_batch_us']:.2f} us"
    )
    print(
        f"Maximum batch: "
        f"{summary['maximum_batch_us']:.2f} us"
    )
    print(
    ...
    print(
        f"Core plus nonfinal transitions: "
    ...
    print(
        f"Speedup versus previous same-size batch: "
        f"{summary['speedup_vs_previous_batch']:.3f}x"
    )
    ...
    if (
        ...
        print(
            "PASS: prefetched FPGA median is "
            f"{summary['speedup_vs_openfhe']:.3f}x "
            "faster than OpenFHE"
        )
    else:
        print(
            "INFO: prefetched FPGA median is "
...
def main() -> None:
    ...
    print(
        ...
        "for the complete prefetch sweep"
    )
    print(
        f"Profile frame: {PROFILE_BYTES} bytes"
    )
    print(
        f"Profile load: {profile_us:.2f} us"
    )
    ...
    for batch_count in batch_counts:
        ...
        try:
            ...
            final0, final1 = validate_batch(
                receive_buffer,
                expected,
                f"Prefetch batch {batch_count} first run",
            )

            print()
            print(
                f"PASS: prefetch batch count {batch_count} "
            ...
            print(
                ...
                f"prefetch batches of count {batch_count} "
                "all match OpenFHE"
            )
    ...
    csv_path = output_root / "prefetch_sweep.csv"
    json_path = output_root / "prefetch_sweep.json"
    ...
    print(
        f"Best speedup versus old batching: "
        f"{best['speedup_vs_previous_batch']:.3f}x"
    )

    if monotonic:
        print(
            "PASS: amortized median did not increase "
            "as prefetch batch count grew"
        )
    else:
        print(
    ...
    if float(best["median_product_us"]) <= 6_450.0:
        print(
            "PASS: batch-16 prefetch target of "
            "6450 us/product was reached"
        )
    else:
        print(
            "INFO: batch-16 prefetch target of "
            "6450 us/product was not reached"
        )

    print(f"CSV:  {csv_path}")
    print(f"JSON: {json_path}")
    print(
        "PASS board operand-prefetch OpenFHE sweep"
    )
