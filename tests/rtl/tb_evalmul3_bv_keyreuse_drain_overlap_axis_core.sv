...
module tb_evalmul3_bv_keyreuse_drain_overlap_axis_core;
    ...
    integer overlap_handshakes;
    integer drain_overlap_handshakes;
    ...
    begin
        ...
        if (overlap_handshakes == 0)
        begin
            $display(
                "ERROR: simulation observed no simultaneous input/output handshakes"
            );

            $fatal(1);
        end

        if (drain_overlap_handshakes == 0)
        begin
            $display(
                "ERROR: next-coefficient input never overlapped prior-bank compute drain"
            );
        ...
        $display(
            "PINGPONG_OUTPUT_OVERLAP_HANDSHAKES=%0d",
            overlap_handshakes
        );

        $display(
            "PINGPONG_DRAIN_OVERLAP_HANDSHAKES=%0d",
            drain_overlap_handshakes
        );
