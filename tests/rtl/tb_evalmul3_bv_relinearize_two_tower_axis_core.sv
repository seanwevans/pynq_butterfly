...
module tb_evalmul3_bv_relinearize_two_tower_axis_core;
    ...
    localparam integer PAYLOAD_WORDS_PER_COEFFICIENT =
        4 + 3 * DIGIT_COUNT;

    localparam integer PAYLOAD_WORDS_PER_PAIR =
        N * PAYLOAD_WORDS_PER_COEFFICIENT;
    ...
    evalmul3_bv_relinearize_two_tower_axis_core #(
        .N                 (N),
        .MAX_DIGITS        (16),
        .INNER_FIFO_DEPTH  (8),
        .ALIGN_FIFO_DEPTH  (8)
    ...
    begin
        $display(
            "PROGRESS: pair %0d loading fused profile",
            pair_index
        );
        ...
        $display(
            "PROGRESS: pair %0d sending fused RLBV payload",
            pair_index
        );
    ...
    begin
        ...
        begin
            ...
            begin
                $display(
                    "ERROR: fused relinearization watchdog fired"
                );
    ...
    begin
        ...
        begin
            /*
             * $strobe prints after nonblocking reset assignments settle, so
             * state should be STATE_IDLE on the first reset clock edge.
             */
            $strobe(
                "TRACE: settled clock edge time=%0t reset_n=%0b state=%0d eval_state=%0d bv_state=%0d",
                $time,
                reset_n,
                dut.state,
                dut.evalmul_engine.input_state,
                dut.bv_mac_engine.input_state
            );
        end
    end
    ...
    begin
        ...
        $display(
            "PROGRESS: loading exact OpenFHE fused vectors from %s",
            vector_root
        );
        ...
        $display(
            "PROGRESS: profile vectors loaded at simulation time %0t",
            $time
        );
        ...
        $display(
            "PROGRESS: payload vectors loaded at simulation time %0t",
            $time
        );
        ...
        $display(
            "PROGRESS: expected vectors loaded at simulation time %0t",
            $time
        );
        ...
        $display(
            "PROGRESS: simulation time advanced beyond zero"
        );
