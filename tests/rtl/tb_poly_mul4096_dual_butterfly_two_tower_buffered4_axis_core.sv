...
module tb_poly_mul4096_dual_butterfly_two_tower_buffered4_axis_core;
...
`endif
    ...
    poly_mul4096_dual_butterfly_two_tower_buffered_axis_core dut (
        .clk                           (clk),
        .reset_n                       (reset_n),
    ...
    begin
        ...
        begin
            ...
            begin
                $display(
                    "PROGRESS BUFFERED4 elapsed=%0d products=%0d prefetches=%0d refills=%0d input=%0d output=%0d",
                    elapsed_cycles,
                    completed_products,
            ...
            begin
                $display(
                    "FAIL BUFFERED4 WATCHDOG elapsed=%0d products=%0d prefetches=%0d refills=%0d protocol_error=%0d",
                    elapsed_cycles,
                    completed_products,
                    completed_prefetches,
                    completed_refills,
    ...
    begin
        ...
        $display("START: four-wide handoff with decoupled result output");
        ...
        if (completed_handoffs !== BATCH_COUNT)
        begin
            $display("FAIL HANDOFF COUNT result=%0d", completed_handoffs);
            $fatal(1);
        end

        if (last_handoff_cycles < 32'd1024 || last_handoff_cycles > 32'd1030)
        begin
            $display(
                "FAIL HANDOFF CYCLES result=%0d expected_range=1024..1030",
                last_handoff_cycles
            );
            $fatal(1);
        end
        ...
        begin
            $display("FAIL: next computation never overlapped prior output");
            $fatal(1);
        end
        ...
        $display("PASS: four coefficients were captured and refilled per handoff clock");
        $display("PASS: next computation began before prior result transmission finished");
        ...
        $display("PASS: expected additional result storage is 8 RAMB36 across two towers");
        $display("Prefetched products: %0d", completed_prefetches);
        $display("Four-wide refills: %0d", completed_refills);
        $display("Buffered handoffs: %0d", completed_handoffs);
        $display("Last handoff cycles: %0d", last_handoff_cycles);
