...
WORK_DIR="${BV_KEYREUSE_COEFFICIENT_MAJOR_DMA150_WORK:-/mnt/f/v/bv_keyreuse_coefficient_major_dma150_overlay}"
DEPLOY_DIR="$ROOT/deploy/bv_keyreuse_coefficient_major_dma150"
...
required=(
  ...
  "$ROOT/rtl/evalmul3_bv_keyreuse_coefficient_major_dma_wrapper.v"
  "$ROOT/scripts/vivado/build_bv_keyreuse_coefficient_major_dma150_overlay.tcl"
)
...
rm -f \
  "$DEPLOY_DIR/evalmul3_bv_keyreuse_coefficient_major_dma150.bit" \
  "$DEPLOY_DIR/evalmul3_bv_keyreuse_coefficient_major_dma150.hwh" \
...
tcl_windows="$repo_windows\\scripts\\vivado\\build_bv_keyreuse_coefficient_major_dma150_overlay.tcl"

cmd_file="$WORK_DIR/run_bv_keyreuse_coefficient_major_dma150_overlay.cmd"
...
marker='PASS: coefficient-major evaluation-key-reuse dual-clock DMA overlay routed'

if [[ ! -f "$summary" ]] \
  || ! tr -d '\r' <"$summary" | grep -Fxq "$marker"; then
  echo "error: Vivado did not record the coefficient-major overlay PASS marker" >&2
  status=1
fi
