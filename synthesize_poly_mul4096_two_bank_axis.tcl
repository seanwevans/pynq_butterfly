...
set model_dir [file join $root_dir model golden_n4096]

set project_root [file join $root_dir vivado poly_mul4096_two_bank_axis_synth]
set report_dir [file join $root_dir reports poly_mul4096_two_bank_axis_synth]

set project_name poly_mul4096_two_bank_axis_synth
set top_name poly_mul4096_two_bank_axis_core
...
set rtl_files [list \
    ...
    [file join $rtl_dir ntt4096_twiddle_rom.sv] \
    [file join $rtl_dir ntt4096_factor_rom.sv] \
    [file join $rtl_dir ntt4096_coeff_bram.sv] \
    [file join $rtl_dir poly_mul4096_two_bank_core.sv] \
    [file join $rtl_dir poly_mul4096_two_bank_axis_core.sv] \
]

set memory_files [list \
    [file join $model_dir forward_twiddles.mem] \
    [file join $model_dir inverse_twiddles.mem] \
    [file join $model_dir twist_factors.mem] \
    [file join $model_dir inverse_scale_factors.mem] \
]
...
set xdc_file [file join $project_root poly_mul4096_two_bank_axis_synth.xdc]
...
if {![string match "*Complete*" $synthesis_status]} {
    error "N=4096 two-bank AXI4-Stream synthesis failed: $synthesis_status"
}

open_run synth_1

set utilization_report \
    [file join $report_dir poly_mul4096_two_bank_axis_utilization.rpt]

set hierarchical_report \
    [file join $report_dir poly_mul4096_two_bank_axis_hierarchical_utilization.rpt]

set timing_report \
    [file join $report_dir poly_mul4096_two_bank_axis_timing.rpt]
...
set summary_file \
    [file join $report_dir poly_mul4096_two_bank_axis_synthesis_summary.txt]
...
puts $fh "coefficient_banks=2"
puts $fh "input_frame_words=8192"
puts $fh "input_frame_bytes=32768"
puts $fh "output_frame_words=4096"
puts $fh "output_frame_bytes=16384"
puts $fh "core_cycles=1339394"
puts $fh "core_latency_us_at_100mhz=13393.94"
puts $fh "modular_multiplications=90112"
puts $fh "expected_ramb36=24"
...
puts ""
puts "============================================================"
puts "N=4096 TWO-BANK AXI4-STREAM SYNTHESIS COMPLETE"
puts "============================================================"
puts "LUTs:                  $lut_count"
...
puts "Core latency:          13393.94 us at 100 MHz"
